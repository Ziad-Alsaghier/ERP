...
class BankAccountController extends Controller
{
    ...
    {
        if (\Auth::user()->can('create bank account')) {
            // الحصول على الحسابات الرئيسية بدون الترجمة في الاستعلام
            $chartAccounts = ChartOfAccount::select('code', 'name', 'id')
                ->where('parent', '=', 0)
                ->where('created_by', \Auth::user()->creatorId())
                ->get()
                ->mapWithKeys(function ($item) {
                    // إعداد code_name مع code و name المترجمين
                    return [$item['id'] => $item['code'] . ' - ' . __($item['name'])];
                });
    
            $chartAccounts->prepend(__('Select Account'), 0);
    
            // الحصول على الحسابات الفرعية بدون الترجمة في الاستعلام
            $subAccounts = ChartOfAccount::select('chart_of_accounts.code', 'chart_of_accounts.name', 'chart_of_accounts.id', 'chart_of_account_parents.account')
                ...
                ->map(function ($item) {
                    // إعداد code_name مع code و name المترجمين
                    $item['code_name'] = $item['code'] . ' - ' . __($item['name']);
                    return $item;
                })
                ->toArray();
