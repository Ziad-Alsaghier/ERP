 ...
 protected $accountsService;
    ...
    public function index(Request $request)
    {
        if(\Auth::user()->can('manage bill'))
        {
            ...
            $query = Bill::where('type', '=', 'Bill')->where('created_by', '=', \Auth::user()->creatorId())->orderBy('id', 'desc');
    ...
    public function create($vendorId)
    {
        ...
        if(\Auth::user()->can('create bill'))
        {
            ...
            $chartAccounts = ChartOfAccount::select('code', 'name', 'id')
            ->where('created_by', \Auth::user()->creatorId())
            ->get()
            ->mapWithKeys(function ($item) {
                return [
                    $item->id => $item->code . ' - ' . __('' . $item->name)
                ];
            });
    ...
    public function sent($id)
    {
        if(\Auth::user()->can('send bill'))
        {
            ...
            foreach ($bill_products as $bill_product) {
                ...
                $itemAmount = ($bill_product->price * $bill_product->quantity) - ($bill_product->discount) + $totalTaxPrice;
                $costAmount = $bill_product->price * $bill_product->quantity;
            ...
            $this->transactionToAccountsPaypal($bill);
            if($bill->getTotalTax() > 0){
                $this->transactionToVatPayable($bill);
            }
//            dd($vendorArr);
...
//        }
    ...
    public function payment($bill_id)
    {
        if(\Auth::user()->can('create payment bill'))
        {
            ...
            $miniTypes = $this->accountsService->getMiniTypesByCategories(['money']);
            ...
            return view('bill.payment', compact('venders', 'categories', 'accounts', 'bill','accounts_type','accounts','subAccounts'));
        }
        else
        {
            return redirect()->back()->with('error', __('Permission denied.'));
    ...
    public function createPayment(Request $request, $bill_id)
    {
        ...
        if(\Auth::user()->can('create payment bill'))
        {
            ...
            // muhamed 18-11
            if($billPayment){
                $account = ChartOfAccount::getAnAcount('Accounts payable');
                ...
                $data = [
                    ...
                    'reference_sub_id' => $billPayment->bill_id,
    ...
    public function duplicate($bill_id)
    {
        if(\Auth::user()->can('duplicate bill'))
        {
            ...
            Notification::create([
                ...
                'data' => json_encode([
                    'action' => 'duplicate',
                    'bill_id' => $duplicateBill->bill_id,
                    'vender_id' => $duplicateBill->vender_id,
                    'bill_date' => $duplicateBill->bill_date,
                    'status' =>$duplicateBill->status,
                    'type' => $duplicateBill->type,
                    'user_type' => $duplicateBill->user_type,
                    'due_date' => $duplicateBill->due_date,
                    'category_id' => $duplicateBill->category_id,
                    'order_number' => $duplicateBill->order_number
                ]),
                'is_read' => 0,
            ]);
...
//        $img          = asset($logo . '/' . (isset($company_logo) && !empty($company_logo) ? $company_logo : 'logo-dark.png'));
        ...
        if(isset($bill_logo) && file_exists(public_path($bill_logo)) && !empty($bill_logo))
        {
            $img = Utility::get_file('bill_logo/') . $bill_logo;
        }
        else{
...
// muhamed 18-11
    private function transactionToAccountsPaypal($bill)
    {
        $name = 'Accounts payable'; // Account name
        $type = 'Credit'; // Receivables are debited (asset)
        $amount = $bill->getTotal() ?? 0; // Total amount excluding VAT
        $ref = 'Bill'; // Reference
        $refId = $bill->id ?? 0; // bill ID
        $refSubId = 0; // Optional
        $date = $bill->issue_date ?? now()->toDateString(); // bill date
    ...
    // private function transactionFromAccountsPaypal($bill)
    // {
    //     $name = 'Accounts payable'; // Account name
    //     $type = 'Debit'; // Receivables are debited (asset)
    //     $amount = $bill->amount ?? 0; // Total amount excluding VAT
    //     $ref = 'Bill'; // Reference
    //     $refId = $bill->id ?? 0; // bill ID
    //     $refSubId = 0; // Optional
    //     $date = $bill->issue_date ?? now()->toDateString(); // bill date
    ...
    private function transactionToVatPayable($bill)
    {
        $name = 'VAT payable'; // Account name
        $type = 'Debit'; // VAT is credited (liability)
        $amount = $bill->getTotalTax() ?? 0; // Extract the tax amount
        $ref = 'Bill '; // Reference
        $refId = $bill->id ?? 0; // bill ID
        $refSubId = 0; // Optional
        $date = $bill->issue_date ?? now()->toDateString(); // bill date
