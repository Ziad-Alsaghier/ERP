    ...
    public function update(User $client, Request $request)
    {
        if(\Auth::user()->can('edit client'))
        {
            ...
            if($client->created_by == $user->creatorId())
            {
                ...
                if($client->name == $request->name && isset($existingUser->email) && $request->email == $existingUser->email ||
                $client->name !== $request->name && $client->email !== $request->email && isset($existingUser->email) && $request->email == $existingUser->email 
                ){
                    if ($existingUser) {
                        return redirect()->back()->with('error', __('A user with the same name, email, and type already exists.'));
                    }
