...
class CustomerController extends Controller
{
    ...
    public function datajson(Request $request)
    {
        ...
        $totalData = Customer::where('created_by', \Auth::user()->creatorId())->count();
        ...
        if(empty($request->input('search.value')))
        {
            $customers = Customer::where('created_by', \Auth::user()->creatorId())
                        ->offset($start)
                        ->limit($limit)
                        ->orderBy('id', 'desc') // ترتيب تنازلي باستخدام id أو created_at
                        ->get();
        }
        else {
            ...
            $totalFiltered = Customer::where('created_by', \Auth::user()->creatorId())
                            ->where('id','LIKE',"%{$search}%")
                            ->orWhere('name', 'LIKE',"%{$search}%")
                            ->orWhere('email', 'LIKE',"%{$search}%")
                            ->orWhere('contact', 'LIKE',"%{$search}%")
        ...
        if(!empty($customers))
        {
            foreach ($customers as $customer)
            {
                ...
                $nestedData['contact'] = $customer->contact;
                $nestedData['email'] = $customer->email;
                $nestedData['balance'] = \Auth::user()->priceFormat($customer->balance);

                // action buttons
                if(\Auth::user()->can('show customer')){
                    $nestedData['action'] =  '
                        <div class="action-btn bg-info ms-2">
                            <a class="mx-3 btn btn-sm align-items-center"  href="'.route('customer.show',\Crypt::encrypt($id)) .'" data-ajax-popup="true" data-size="lg"  data-bs-toggle="tooltip" title="'.__('View').'"  data-title="'.__('View Customer').'">
                ...
                if(\Auth::user()->can('edit customer')){
                    $nestedData['action'] .=  '
                        <div class="action-btn bg-primary ms-2">
                            <a href="#" class="mx-3 btn btn-sm  align-items-center" data-url="' . route('customer.edit',$id).'" data-ajax-popup="true"  data-size="lg" data-bs-toggle="tooltip" title="'.__('Edit').'"  data-title="'.__('Edit Customer').'">
                    ...
                    if($customer->is_enable_login == 1){
                        $nestedData['action'] .=  '
                            <div class="action-btn bg-warning ms-2">
                                <a href="'. route('customer.login',\Crypt::encrypt($id)) .'" class="mx-3 btn btn-sm align-items-center" data-bs-toggle="tooltip" title="'.__('Login').'">
                                    <i class="ti ti-logout text-white text-white"></i>
                                </a>
                            </div>
                        ';
                    } else {
                ...
                if(\Auth::user()->can('delete customer')){
                    $nestedData['action'] .=  '
                        <div class="action-btn bg-danger ms-2">
                            '. Form::open(['method' => 'DELETE', 'route' => ['customer.destroy', $id],'id'=>'delete-form-'.$id]) .'
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->can('create customer'))
        {
            ...
            if(empty($request->balance) || $request->balance == "" || $request->balance == null){
                $request->balance = 0;
            }
    ...
    public function loginunable($ids){
        $eId = \Crypt::decrypt($ids);
        $user = Customer::find($eId);
        $authUser = \Auth::user();
        // @dd($user , $eId ,$user->is_enable_login);
...
{
    $latestCustomerId = Customer::where('created_by', '=', \Auth::user()->creatorId())->max('customer_id');
