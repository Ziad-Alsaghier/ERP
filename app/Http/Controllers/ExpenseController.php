...
class ExpenseController extends Controller
{
    ...
    public function create($Id)
    {
        if (\Auth::user()->can('create bill')) {
            ...
            // add chart of account in select
            $chart_accounts_type = ChartOfAccountType::select('id', 'name')
            ->where('created_by', \Auth::user()->creatorId())
            ->orderBy('id', 'asc') 
            ->get();
                    
            $chart_accounts = ChartOfAccount::select('chart_of_accounts.id', 'chart_of_accounts.code', 'chart_of_accounts.name', 'chart_of_accounts.type', 'chart_of_accounts.parent')
            ->where('parent', '=', 0)
            ->where('created_by', \Auth::user()->creatorId())
            ...
            $chart_subAccounts->leftJoin('chart_of_account_parents', 'chart_of_accounts.parent', '=', 'chart_of_account_parents.id');
            $chart_subAccounts->where('chart_of_accounts.parent', '!=', 0);
            $chart_subAccounts->where('chart_of_accounts.created_by', \Auth::user()->creatorId());
            $chart_subAccounts = $chart_subAccounts->get()->toArray();
            foreach ($chart_subAccounts as &$subAccount) {
                $subAccount['name'] = $subAccount['code'] . ' - ' . __($subAccount['name']);
            }
            // end add chart of account in select
            ...
            return view('expense.create', compact('employees', 'customers', 'venders', 'expense_number', 'product_services', 'category', 'customFields', 'Id', 'accounts','chart_accounts','chart_accounts_type','chart_subAccounts','chart_subAccounts'));
        } else {
    ...
    public function store(Request $request)
    {
        ...
        if (\Auth::user()->can('create bill')) {
            ...
            foreach ($bill_products as $bill_product) {
                ...
                $product_chart_account = $product->expense_chartaccount_id;

                $data = [
                    'account_id' => $product_chart_account,
    ...
    public function edit($ids)
    {
        ...
        if (\Auth::user()->can('edit bill')) {
            ...
            $bankAccount = BankAccount::find($bankAccount->account_id ?? 0);
            ...
            if ($expense->created_by == \Auth::user()->creatorId()) {
                ...
                for ($i = 0; $i < count($products); $i++) {
                    ...
                    if ($expenseProduct == null) {
                        ...
                        try{ 
                            /**
                             * Add New Feature Try Catch Block for Error Handling Date :
                             * Date : 2/25/2025  
                              */ 
                            ...
                            if (isset($products[$i]['items'])) {
                                ...
                                \Log::error('Error in updating expense: ' . $e->getMessage());
                            return redirect()->back()->with('error', __('Something is wrong In .'));
                        }
