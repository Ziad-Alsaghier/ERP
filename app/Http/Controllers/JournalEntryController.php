...
class JournalEntryController extends Controller
{
    ...
    public function create()
    {
        if (\Auth::user()->can('create journal entry')) {
            ...
            $accounts_type = $this->accountService->getChartAccountTypes();
            $miniTypes = $this->accountService->getMiniTypesByCategories();
            $totalaccounts = $this->accountService->getChartAccounts();
            $accounts = $totalaccounts['parent_accounts'];
            $subAccounts = $totalaccounts['sub_accounts'];

            $journalId = $this->journalNumber();

            return view('journalEntry.create', compact('accounts', 'subAccounts', 'journalId','accounts_type'));
        } else {
        ...
        if (\Auth::user()->can('create invoice')) {
            ...
            if(!empty($request->attachment))
            {
                ...
                if($result==1)
                {
                    ...
                    {
                        $path = storage_path('uploads/attachments' . $journal->attachment);
                        if(file_exists($path))
                        {
                            \File::delete($path);
                        }
    ...
    public function edit(JournalEntry $journalEntry)
    {
        if (\Auth::user()->can('edit journal entry')) {
            $chartAccounts = ChartOfAccount::select(\DB::raw('CONCAT(chart_of_accounts.name) AS code_name,chart_of_accounts.id, chart_of_accounts.code,  chart_of_accounts.parent'))
                ->where('parent', '=', 0)
                ->where('created_by', \Auth::user()->creatorId())->get()
                ->toarray();

            $subAccounts = ChartOfAccount::select(\DB::raw('CONCAT(chart_of_accounts.name) AS code_name , chart_of_accounts.id, chart_of_accounts.code , chart_of_account_parents.account'));
