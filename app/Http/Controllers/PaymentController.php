...
//            $categories = ProductServiceCategory::where('created_by', '=', \Auth::user()->creatorId())->where('type', '=', 2)->get()->pluck('name', 'id');
    ...
    {
        $latest = Payment::where('created_by', '=', \Auth::user()->creatorId())->latest()->first();
        if (!$latest) {
            return 1;
        }
...
//                    $fileName = time() . "_" . $request->add_receipt->getClientOriginalName();
            ...
            if($payment){
                ...
                $data = [
                    ...
                    'reference_sub_id' => $payment->bill_id ??  $payment->payment_id,
                    'date' => $payment->date,
                ];
                $dataMoney = [
...
//             ];
    ...
    public function show(Payment $payment)
    {
        ...
        if ($payment->created_by == \Auth::user()->creatorId()) {
            ...
            return view('payment.show', compact('venders', 'categories', 'accounts', 'payment', 'chartAccounts'));
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
...
//                        }
    ...
    public function payment($payment_id){
        ...
        if(!empty($payment) && $payment->created_by == \Auth::user()->creatorId()){


        if(!empty($payment)){
            return view('payment.pdf', compact('payment','settings'));
        }else{
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }else{
