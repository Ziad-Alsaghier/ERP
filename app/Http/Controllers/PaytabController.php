...
class PaytabController extends Controller
{
    ...
    public function planPayWithpaytab(Request $request)
    {
        ...
        try {
            ...
            $this->paymentconfig();
            $user = Auth::user();
            $config = config('paytabs.currency');
            if ($plan) {

                $users = $request->Users ?? 0;
                $customers = $request->Customers ?? 0;
                $venders = $request->Venders ?? 0;
                $clients = $request->Clients ?? 0;

                $total_price_user = $plan->user_price * $users ?? 0;
                $total_price_customer = $plan->customers_price * $customers ?? 0;
                $total_price_vender = $plan->venders_price * $venders ?? 0;
                $total_price_client = $plan->clients_price * $clients ?? 0;

                $total = $plan->price + $total_price_user + $total_price_customer + $total_price_vender + $total_price_client;
                $exchangeRate = 50;
                $get_amount = $total * $exchangeRate;

                if (!empty($request->coupon)) {
                    $coupons = Coupon::where('code', strtoupper($request->coupon))->where('is_active', '1')->first();
                    if (!empty($coupons)) {
                        $usedCoupun = $coupons->used_coupon();
                        $discount_value = ($total / 100) * $coupons->discount;
                        $get_amount = $total - $discount_value;
                            ...
                            if ($assignPlan['is_success'] == true && !empty($plan)) {
                                ...
                                Order()::create(
                                    [
                                        ...
                                        'card_number' => $request['cc-number'],
                                        'card_exp_month' => $request['cc-month'],
                                        'card_exp_year' => $request['cc-year'],
                                        'plan_name' => $plan->name,
                                        'plan_id' => $plan->id,
                ...
                $pay = paypage::sendPaymentCode('all')
                    ...
                    \Log::info('pay : ' . $pay . ' , config : ' . $config );
                return $pay;
