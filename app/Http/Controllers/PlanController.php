...
class PlanController extends Controller
{
    ...
    public function index()
    {
        ...
        if(\Auth::user() &&\Auth::user()->can('manage plan'))
        {
            if(\Auth::user()->type == 'super admin')
            {
                ...
                return view('plan.index', compact('plans','users', 'admin_payment_setting'));
    ...
    public function store(Request $request)
    {
        ...
        if(\Auth::user()->can('create plan'))
        {
                    ...
                    || $admin_payment_setting['is_paytr_enabled'] == 'on'))
            {
                ...
                if(isset($request->enable_manufacturing))
                {
                    $post['manfuc'] = 1;
                }
        ...
        if(\Auth::user()->can('edit plan'))
        {
                    ...
                    || $admin_payment_setting['is_paytr_enabled'] == 'on'))
            {
                ...
                if(!empty($plan))
                {
                    ...
                    if(array_key_exists('enable_manufacturing', $post))
                    {
                        $post['manfuc'] = 1;
                    }
                    else
    ...
    public function planVisible(Request $request)
    {


        Plan::where('id', $request->id)->update(['is_visible' => $request->is_visible]);

        if ($request->is_visible == 1) {
            return response()->json(['success' => __('Plan successfully is visible.')]);

        } else {
            return response()->json(['success' => __('Plan successfully is not visible.')]);
        }
    }
}
