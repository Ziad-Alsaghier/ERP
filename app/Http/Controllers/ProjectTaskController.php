        ...
        if (\Auth::user()->can('create project task')) {
            ...
            Notification::create([
                ...
                'data' => json_encode([
                    ...
                    'project_name' => $project->project_name,
                    'user_name' => \Auth::user()->name,
                    'description' => " Task $task->name ". " Created by "  .\Auth::user()->name
                ]),
                'is_read' => 0,
            ]);
            //Slack Notification
...
//                }
    ...
    public function show($project_id, $task_id)
    {
        ...
        if (\Auth::user()->can('view project task')) {
            ...
            $customFields_label = CustomField::where('created_by', '=', \Auth::user()->creatorId())->where('module', '=', 'Project')->get();
            $customFields = CustomFieldValue::where('record_id', '=', [$task_id])->get();


            $allow_progress = Project::find($project_id)->task_progress;
            $task = ProjectTask::find($task_id);

            return view('project_task.view', compact('task', 'allow_progress','customFields_label','customFields'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied.'));
        }
    }
    ...
    public function destroy($project_id, $task_id)
    {
        ...
        if (\Auth::user()->can('delete project task')) {
            ...
            CustomFieldValue::where('record_id', '=', [$task_id])->delete();
