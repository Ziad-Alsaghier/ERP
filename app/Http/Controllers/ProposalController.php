...
class ProposalController extends Controller
{
    ...
    public function index(Request $request)
    {
        if(\Auth::user()->can('manage proposal'))
        {
            ...
            $query = Proposal::where('created_by', '=', \Auth::user()->creatorId())->orderBy('created_at', 'desc');
            ...
            $proposals = $query->with(['category', 'customer', 'user'])->get();
    ...
    public function create($customerId = 0)
    {
        if(\Auth::user()->can('create proposal'))
        {
            ...
            return view('proposal.create', compact('customers', 'proposal_number', 'product_services', 'category', 'customFields', 'customerId', 'orders'));
        }
        else
        {
    ...
    public function store(Request $request)
    {
        if(\Auth::user()->can('create proposal'))
        {
            ...
            $status = Proposal::$statues;
            if(empty($request->proposal_details)){
                $request->due_date = "" ;
            }
            ...
            $proposal->proposal_details     = $request->proposal_details;
            $proposal->project              = $request->make_project;
            ...
            {
                if(isset($products[$i]['type'])){
                    $description = json_decode($products[$i]['description'], true);
                    if (is_array($description) &&  isset($products[$i]['design_note'])) {
                        $description['design_note'] = $products[$i]['design_note'];
                    }
                    if (is_array($description) &&  isset($products[$i]['attachment'])) {
                        $description['attachment'] = $products[$i]['attachment'];
                    }
                    $products[$i]['description'] = json_encode($description);
                }
    ...
    public function destroy(Proposal $proposal)
    {
        if(\Auth::user()->can('delete proposal'))
        {
            if($proposal->created_by == \Auth::user()->creatorId())
            {
                ...
                Project::where('ref', '=', 'proposal')
                ->where('ref_id', '=', $proposal->id)
                ->delete();
...
//                dd($proposalArr);
    ...
    public function project($proposal_id)
    {
            ...
            $proposalProductArray = [];
            foreach ($proposalProduct as $key => $val) {
                $proposalProductArray[$key] = $val;
            }
            $proposalProductJson = json_encode($proposalProductArray);

            $project = new Project();
            $project->project_name = $this->invoiceNumber();
            $project->client_id = $proposal['customer_id'];
            $project->start_date = date('Y-m-d');
            $project->end_date = date('Y-m-d');
            $project->budget = 20;
            $project->ref = 'proposal';
            $project->ref_id = $proposal_id;
            $project->description = $proposalProductJson; // تضمين البيانات JSON هنا
            $project->status = 'in_progress';
            $project->created_by = \Auth::user()->creatorId();
            $project->project_image = 'projects/1711366735.png';
            $project->save();
            ...
            return redirect()->back()->with('success', __('Proposal to project convert successfully.'));
    ...
    public function convert($proposal_id)
    {
        if(\Auth::user()->can('convert invoice'))
        {
            ...
            // dd($convertInvoice->invoice_id);
            $convertInvoice->customer_id = $proposal['customer_id'];
            $convertInvoice->issue_date  = date('Y-m-d\TH:i');
            $convertInvoice->due_date    = date('Y-m-d\TH:i');
            $convertInvoice->send_date   = null;
            $convertInvoice->category_id = $proposal['category_id'];
            $convertInvoice->order_id = $proposal['order_id'];
            $convertInvoice->address     = $proposal['address'];
            ...
            if($convertInvoice)
            {
                ...
                foreach($proposalProduct as $product)
                {
                    ...
                    if (isset($duplicateProduct->type) && $duplicateProduct->type != 'custom') {
    ...
    public function statusChange(Request $request, $id)
    {
        ...
        return response()->json(['message' => __('Proposal status changed successfully.')]);
    }
...
//        }
    ...
    {
        $latestInvoiceId = Invoice::where('created_by', '=', \Auth::user()->creatorId())->max('invoice_id');
    ...
    public function workorder(Request $request){
        ...
        return view('workorder.detailspdf',compact('description'));
    }
}
