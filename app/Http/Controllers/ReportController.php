...
class ReportController extends Controller
{
                ...
                if ($billProduct->tax != null) {
                    foreach (explode(',', $billProduct->tax) as $tax) {
                        if (isset($getTaxData[$tax])) {
                            ...
                            error_log("Tax key $tax not found in getTaxData.");
                        }
                    }
                }
                if (isset($billProduct->month)) {
                    $expenseTaxesData[$billProduct->month][] = $billTax;
                } else {
                    // Debugging information
                    error_log("Month is not set for billProduct.");
                }
            }
        ...
        if (\Auth::user()->can('statement report')) {
            ...
            $account = BankAccount::where('created_by', '=', \Auth::user()->creatorId())
                ...
                ->mapWithKeys(function ($item) {
                    return [$item->id => $item->holder_name . ' - ' . $item->bank_name];
                });
    ...
    public function balanceSheet(Request $request, $view = '', $collapseview = 'expand')
    {
        if (\Auth::user()->can('bill report')) {
            ...
            $subtypes = ChartOfAccountSubType::where('created_by', '=', \Auth::user()->creatorId())->orderBy('id', 'asc')->get();
            ...
            return view('report.balance_sheet', compact('groupedChartAccounts','chartAccounts','subAccounts', 'types','subtypes' , 'filter'));
    ...
    public function ledgerSummary(Request $request, $account = '')
    {
        ...
        if (\Auth::user()->can('ledger report')) {
            ...
            // filter
            $accountCondition = !empty($request->account) ? ['id' => $request->account] : [];
            $chart_accounts = ChartOfAccount::where($accountCondition)
                ->where('created_by',  \Auth::user()->creatorId())
                ->get();

            // selection of accounts
            $accounts_type = ChartOfAccountType::select('id', 'name')
                ->where('created_by', \Auth::user()->creatorId())
                ->orderBy('id', 'asc') // ترتيب تصاعدي بناءً على id
                ->get();

            $accounts = ChartOfAccount::select('chart_of_accounts.id', 'chart_of_accounts.code', 'chart_of_accounts.name', 'chart_of_accounts.type', 'chart_of_accounts.parent')
                ->where('parent', '=', 0)
                ->where('created_by', \Auth::user()->creatorId())
                ->orderBy('chart_of_accounts.code', 'asc') // ترتيب تصاعدي بناءً على code
                ->get()
                ->toArray();
            ...
            $account_customer = Customer::select('id', 'name')->where('created_by', \Auth::user()->creatorId())->get();
            $account_Vender = Vender::select('id', 'name')->where('created_by', \Auth::user()->creatorId())->get();
            $account_Employee = Employee::select('id', 'name')->where('created_by', \Auth::user()->creatorId())->get();
            ...
            $filter['type'] = $request->account_type ?? 'revenue';
            $filter['balance'] = $balance;
            $filter['credit'] = $credit;
            $filter['debit'] = $debit;
            $filter['startDateRange'] = $start;
            $filter['endDateRange'] = $end;
            return view('report.ledger_summary', compact('filter', 'chart_accounts', 'accounts', 'accounts_type', 'subAccounts', 'account_customer', 'account_Vender', 'account_Employee'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied.'));
        }
    }
    ...
    public function trialBalanceSummary(Request $request, $view = "expand")
    {
        if (\Auth::user()->can('trial balance report')) {
            ...
            // dd($subAccounts);
            // dd($groupedChartAccounts);
            return view('report.trial_balance', compact('groupedChartAccounts','chartAccounts','subAccounts', 'types','subtypes' , 'filter'));
        } else {
            return redirect()->back()->with('error', __('Permission Denied.'));
        }
    }
    ...
    public function profitLoss(Request $request, $view = '', $collapseView = 'expand')
    {
        if (\Auth::user()->can('income vs expense report')) {
            ...
            // dd($subAccounts);
            // dd($groupedChartAccounts);
            return view('report.profit_loss', compact('groupedChartAccounts','chartAccounts','subAccounts', 'types','subtypes' , 'filter'));
    ...
    public function monthlyCashflow(Request $request)
    {
        ...
        if (\Auth::user()->can('loss & profit report')) {
            ...
            $incomes = Revenue::selectRaw('sum(revenues.amount) as amount,MONTH(date) as month,YEAR(date) as year,category_id')
                ->leftjoin('product_service_categories', 'revenues.category_id', '=', 'product_service_categories.id')->where('product_service_categories.type', '=', 'income');
            $incomes = $incomes->where('revenues.created_by', '=', \Auth::user()->creatorId());
            $incomes = $incomes->whereRAW('YEAR(date) =?', [$year]);
            ...
            $invoiceTotalArray = [];
            foreach ($invoices as $invoice) {
                $invoiceTotalArray[$invoice->month][] = $invoice->getDue();
            }
            for ($i = 1; $i <= 12; $i++) {
                $invoiceTotal[] = array_key_exists($i, $invoiceTotalArray) ? array_sum($invoiceTotalArray[$i]) : 0;
            }
            ...
            //   -----------------------------------------PAYMENT EXPENSE ------------------------------------------------------------
            $expenses = Payment::selectRaw('sum(payments.amount) as amount,MONTH(date) as month,YEAR(date) as year,category_id')->leftjoin('product_service_categories', 'payments.category_id', '=', 'product_service_categories.id')->where('product_service_categories.type', '=', 'expense');
            $expenses->where('payments.created_by', '=', \Auth::user()->creatorId());
            $expenses->whereRAW('YEAR(date) =?', [$year]);
    ...
    public function date_Cashflow(Request $request)
    {
        ...
        if (\Auth::user()->can('loss & profit report')) {
            ...
            // ------------------------------REVENUE INCOME-----------------------------------
            $incomes = Revenue::selectRaw('sum(revenues.amount) as amount,category_id , date ,product_service_categories.name as category')
                ->leftjoin('product_service_categories', 'revenues.category_id', '=', 'product_service_categories.id')->where('product_service_categories.type', '=', 'income');
            $incomes = $incomes->where('revenues.created_by', '=', \Auth::user()->creatorId());
            $incomes = $incomes->whereBetween('date', [$start, $end]);
            ...
            $chartIncomeArr = number_format($finalAmount, 2);
            ...
            //   -----------------------------------------PAYMENT EXPENSE ------------------------------------------------------------
            $expenses = Payment::selectRaw('sum(payments.amount) as amount,category_id , date ,product_service_categories.name as category ')->leftjoin('product_service_categories', 'payments.category_id', '=', 'product_service_categories.id')->where('product_service_categories.type', '=', 'expense');
            $expenses->where('payments.created_by', '=', \Auth::user()->creatorId());
            $expenses = $expenses->whereBetween('date', [$start, $end]);
            ...
            $expenseTotalFormatted = number_format($expenseTotal, 2);



            //     ------------------------------------BILL EXPENSE----------------------------------------------------

            $bills = Bill::selectRaw('category_id,bill_id,bills.id,product_service_categories.name as category')->where('bills.created_by', \Auth::user()->creatorId())->where('status', '!=', 0)->leftjoin('product_service_categories', 'bills.category_id', '=', 'product_service_categories.id')->where('product_service_categories.type', '=', 'expense');;
            $bills = $bills->whereBetween('send_date', [$start, $end]);
            ...
            foreach ($bills as $bill) {
                ...
                if (!isset($billTmpArray[$category])) {
                    $billTmpArray[$category] = [
                        'total' => 0,
                        'due' => 0,
                    ];
                }
                $billTmpArray[$category]['total'] += $bill->getTotal();
                $billTmpArray[$category]['due'] += $bill->getDue();
            }
            ...
            foreach ($billTmpArray as $category => $values) {
                $difference = $values['total'] - $values['due'];
                $billArray[] = [
                    'category' => $category,
                    'data' => number_format($difference, 2),
                ];
                $billTotal += $difference;
            }

            $chartExpenseArr = $expenseTotalFormatted + $billTotal;
            $chartIncomeArrNum = (float)str_replace(',', '', $chartIncomeArr);
            $chartExpenseArrNum = (float)str_replace(',', '', $chartExpenseArr);
            // dd(['ex'=>$chartExpenseArrNum , 'in'=>$chartIncomeArrNum]);
            $netProfit = $chartIncomeArrNum - $chartExpenseArrNum;
            $data['chartExpenseArr'] = $chartExpenseArr;
            $data['expenseArr'] = $array;
            $data['billArray'] = $billArray;
            ...
            return view('report.date_cashflow', compact('filter'), $data);
        } else {
            return redirect()->back()->with('error', __('Permission denied.'));
        }
    }
    ...
    // public function dateCashflow(Request $request)
    // {
    //     if (\Auth::user()->can('loss & profit report')) {
    ...
    //     // $startDate = Carbon::createFromDate(2023, 1, 1); // January 1, 2023
    //     // $endDate = Carbon::now();
    //     $netIncome = $this->reportService->getEpit($startDate, $endDate);
    //     $accounts = ChartOfAccount::where('created_by', \Auth::user()->creatorId())
    //         ->with('miniType')
    //         ->get();

    //     // Group accounts by mini_type, handling cases where mini_type is not valid
    //     $groupedAccounts = $accounts->groupBy(function ($account) use ($startDate, $endDate) {
    //         // Check if mini_type is valid and greater than 0
    //         if ($account->mini_type > 0 && $account->miniType) {
    //             $account['balance'] = $account->getBalanceAccount($account, $startDate, $endDate);
    //             return $account->miniType->name; // Group by the name of the mini_type
    //         } else {
    //             return 'not_assigned'; // Handle accounts with no valid mini_type
    //         }
    //     });

    //     $type = TransactionLines::where('created_by', \Auth::user()->creatorId())->get();

    //     $data =  $this->reportService->cashFlowReportByAccounts($groupedAccounts, $netIncome);
    //     // return response($type);
    //     return view('report.date_cashflow', compact('filter','data'), $data);
    // } else {
    //     return redirect()->back()->with('error', __('Permission denied.'));
    // }
    // }
    public function dateCashflow(Request $request)
    {
        if (\Auth::user()->can('loss & profit report')) {
            ...
            $chart_accounts = ChartOfAccount::where('created_by',  \Auth::user()->creatorId())
                ->get();
            if (!empty($request->start_date) && !empty($request->end_date)) {
                $start = $request->start_date;
                $end = $request->end_date;
            ...
            // dd($subAccounts);
            // dd($groupedChartAccounts);
            return view('report.date_cashflow', compact('chart_accounts','groupedChartAccounts','chartAccounts','subAccounts', 'types','subtypes' , 'filter'));
    } else {
        return redirect()->back()->with('error', __('Permission denied.'));
    }
    }
                ...
                foreach ($parentAccounts as $parentAccount) {
                    ...
                    if (!empty($parentAccountArray)) {
                        ...
                        $dataTotal = [
                            ...
                            'account_code' => '',
                            'account' => 'parentTotal',
                            'account_name' =>  $parentAccount->name,
        ...
        foreach ($types as $type) {
            ...
            foreach ($subTypes as $subType) {
                ...
                if ($accountArray != []) {
                    ...
                    $dataTotal['account_code'] = '';
                    $dataTotal['account'] = '';
                    $dataTotal['account_name'] =  $subType->name;
                    $dataTotal['totalCredit'] = $creditTotal;
                    $dataTotal['totalDebit'] = $debitTotal;
        ...
        if (\Auth::user()->can('income vs expense report')) {
            ...
            foreach ($types as $type) {
                ...
                if ($accountArray != []) {
                    ...
                    $dataTotal['account_code'] = '';
                    $dataTotal['account'] = '';
                    $dataTotal['account_name'] =  $type->name;
                    $dataTotal['totalCredit'] = $creditTotal;
                    $dataTotal['totalDebit'] = $debitTotal;
                    $dataTotal['netAmount'] = $totalAmount;
                    $accountArray[][] = $dataTotal;
    ...
    //  Start Show All Reports or Financial Reports
    public function reports(){
        ...
        $reports = [
            'financial'=> [
                ['icon' => 'ti-book', 'title' => 'Account Statement', 'description' => 'Shows the summary of transactions for a specific account over a period, including any balances.', 'route' => 'report.ledger', 'status' => true],
                ['icon' => 'ti-file-invoice', 'title' => 'Account Summary', 'description' => 'A high-level view of an account’s status, showing key balances and recent activity.', 'route' => 'report.account.statement', 'status' => true],
                ['icon' => 'ti-receipt', 'title' => 'Invoice Summary', 'description' => 'Overview of sales transactions, with totals and trends displayed.', 'route' => 'report.invoice.summary', 'status' => true],
                ['icon' => 'ti-clipboard', 'title' => 'Bill Summary', 'description' => 'Overview of purchase transactions, with totals and trends displayed.', 'route' => 'report.bill.summary', 'status' => true],
                ['icon' => 'ti-chart-bar', 'title' => 'Sales Report', 'description' => 'Displays product purchase performance over time using a trend line.', 'route' => 'report.sales', 'status' => true],
                ['icon' => 'ti-wallet', 'title' => 'Receivables', 'description' => 'Accounts receivable reports show the amounts a company owes to suppliers or other parties, including account names, due dates, and amounts due over a specific period.', 'route' => 'report.receivables', 'status' => true],
                ['icon' => 'ti-credit-card', 'title' => 'Payables', 'description' => 'Accounts payable reports show the amounts a company owes to suppliers or other parties, including account names, due dates, and amounts due over a specific period.', 'route' => 'report.payables', 'status' => true],
                ['icon' => 'ti-package', 'title' => 'Inventory Stock', 'description' => 'Summary of product distribution and storage across various organization locations.', 'route' => 'report.product.stock.report', 'status' => true],
                ['icon' => 'ti-cash', 'title' => 'Cash Flow', 'description' => 'Details the cash inflows and outflows from operations, investing, and financing activities.', 'route' => 'report.monthly.cashflow', 'status' => true],
                ['icon' => 'ti-exchange', 'title' => 'Transaction', 'description' => 'Transaction report displays all financial activities, including income and expenses, with details like date, type, and amount.', 'route' => 'transaction.index', 'status' => true],
                ['icon' => 'ti-wallet', 'title' => 'Income Summary', 'description' => 'Income Summary report shows the total income generated during a specific period, categorized by different sources.', 'route' => 'report.income.summary', 'status' => true],
                ['icon' => 'ti-wallet', 'title' => 'Expense Summary', 'description' => 'Expense details detail all expenses paid or recorded during a specific period, including the type of expense, date, and amount spent, to help management track spending and budget.', 'route' => 'report.expense.summary', 'status' => true],
                ['icon' => 'ti-arrows-sort', 'title' => 'Income VS Expense', 'description' => 'Income vs Expense report shows a comparison between total income and total expenses over a specific period, helping to understand if the business is making a profit or loss.', 'route' => 'report.income.vs.expense.summary', 'status' => true],
                ['icon' => 'ti-receipt', 'title' => 'Tax Summary', 'description' => 'A form used to file tax returns, summarizing taxable income and tax liabilities.', 'route' => 'report.tax.summary', 'status' => true],
                ['icon' => 'ti-scale', 'title' => 'Balance Sheet', 'description' => 'Presents the financial position of a company, summarizing its assets, liabilities, and equity at a specific point in time.', 'route' => 'report.balance.sheet', 'status' => true],
                ['icon' => 'ti-scale', 'title' => 'Profit & Loss', 'description' => 'Profit & Loss report shows the revenues and expenses over a specific period, highlighting the net profit or loss of the business.', 'route' => 'report.profit.loss', 'status' => true],
                ['icon' => 'ti-scale', 'title' => 'Trial Balance', 'description' => 'A report that lists the balances of all accounts to ensure that total debits equal total credits.', 'route' => 'trial.balance', 'status' => true],
            ],
            'HRM'=>[
                ['id' => 'Payroll','title' => __('Payroll'),'description' => 'Displays details of employee payroll information, including pay periods and amounts.','icon' => 'ti-businessplan','route' => route('report.payroll'), 'status' => true],
                ['id' => 'Leave', 'title' => __('Leave'), 'description' => 'It tracks and records user activities within the system, helping to monitor actions and behaviors.', 'icon' => 'ti-calendar', 'route' => route('report.leave'), 'status' => true],
                ['id' => 'MonthlyAttendance', 'title' => __('Monthly Attendance'), 'description' => 'Monthly Attendance report shows employees’ attendance and absence for a specific month, including details of present and absent days.', 'icon' => 'ti-calendar-time', 'route' => route('report.monthly.attendance'), 'status' => true]
            ],
            'CRM'=>[
                ['id' => 'Leads', 'title' => __('Leads'), 'description' => 'Displays the number of new customers acquired over time.', 'icon' => 'ti-user-check', 'route' => route('leads.index'), 'status' => true],
                ['id' => 'Deals', 'title' => __('Deals'), 'description' => 'Deals report displays details of all business transactions, including clients, amounts, and the status (closed or in progress).', 'icon' => 'ti-clipboard-check', 'route' => route('deals.index'), 'status' => true],
            ],

            'POS' => [
                ['id' => 'Warehouse Repo', 'title' => __('Warehouse Report'), 'description' => 'POS Stock Inventory Report displays the stock status at point-of-sale locations, including available quantities of products at each POS and products that have been sold or restocked.', 'icon' => 'ti-home', 'route' => route('report.warehouse'), 'status' => true],
                ['id' => 'Purchase Daily', 'title' => __('Purchase Daily/Monthly Report'), 'description' => 'Daily/Monthly Purchase Report displays details of purchases made within a specific day or month, including supplier names, quantities purchased, and associated costs.', 'icon' => 'ti-shopping-cart', 'route' => route('report.daily.purchase'), 'status' => true],
                ['id' => 'POS Dai', 'title' => __('POS Daily/Monthly Report'), 'description' => 'Daily/Monthly POS Report displays the total sales made at point-of-sale locations within a specific day or month, including revenue, number of transactions, and products sold.', 'icon' => 'ti-receipt', 'route' => route('report.daily.pos'), 'status' => true],
                ['id' => 'Pos', 'title' => __('Pos VS Purchase Report'), 'description' => 'POS Purchases/Sales Report displays details of purchases and sales made at point-of-sale locations, including quantities of products sold or purchased, revenue, and associated suppliers or customers.', 'icon' => 'ti-exchange', 'route' => route('report.pos.vs.purchase'), 'status' => true],
            ]
        ];

        foreach ($reports as $category => &$reportList) {
            foreach ($reportList as &$report) {
                $report['status'] = true; // Set the status dynamically as needed
            }
        }
        return view('report.financial_reports',compact('parentMenu','reports'));

    }


    // End Show All Reports or Financial Reports
}
