...
class SystemController extends Controller
{
    ...
    public function saveAccountingSettings(Request $request)
    {
        ...
        $settings = collect(Utility::settingsById(Auth::user()->creatorId())); // Convert to collection
        $closeYearDate = isset($settings['close_year_date']) ? $settings['close_year_date'] : ''; // Find the setting by name

        // Ensure the close_year_date is a valid date
        if (!empty($closeYearDate)) {
            // Make sure both dates are in the same timezone
            $closeYearDate = Carbon::parse($closeYearDate)->timezone('UTC'); // Assuming the stored date is in UTC
        }

        $currentDate = Carbon::now()->timezone('UTC'); // Ensure current date is also in UTC

        // Debugging: Log or dump the values for verification
        \Log::info('Current Date: ' . $currentDate);
        \Log::info('Close Year Date: ' . $closeYearDate);

        // Check if the current date is greater than or equal to the close year date
        if ($currentDate->greaterThanOrEqualTo($closeYearDate)) {
            return redirect()->back()->with('error', __('لا يمكن تعديل الإعدادات بعد إغلاق السنة المالية.'));
        }
        $userId = auth()->id(); // Get the ID of the authenticated user

        // Insert or update each setting using `ON DUPLICATE KEY UPDATE`
        \DB::insert(
            'INSERT INTO settings (`value`, `name`, `created_by`) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE `value` = VALUES(`value`)',
            [$request->input('close_year_date'), 'close_year_date', $userId]
        );

        \DB::insert(
            'INSERT INTO settings (`value`, `name`, `created_by`) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE `value` = VALUES(`value`)',
            [$request->input('start_year_day'), 'start_year_day', $userId]
        );

        \DB::insert(
            'INSERT INTO settings (`value`, `name`, `created_by`) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE `value` = VALUES(`value`)',
            [$request->input('start_year_month'), 'start_year_month', $userId]
        );

        \DB::insert(
            'INSERT INTO settings (`value`, `name`, `created_by`) VALUES (?, ?, ?) ON DUPLICATE KEY UPDATE `value` = VALUES(`value`)',
            [$request->input('inventory_cost_way'), 'inventory_cost_way', $userId]
        );
    ...
    public function companyIndex(Request $request)
    {
        ...
        // $data = Utility::getSetting(\Auth::user()->creatorId());
        // dd($data);
        if (\Auth::user()->can('manage company settings')) {
