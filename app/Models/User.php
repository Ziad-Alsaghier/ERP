...
class User extends Authenticatable implements MustVerifyEmail
{
    ...
    {
        return $this->hasMany('App\Models\LoginDetail', 'user_id', 'id');
    }
    ...
    public function ordersNumberFormat($number){
    ...
    public function priceFormat($price,$decimal_number_lev = 2)
    {
        ...
        if($decimal_number_lev == 2){
            ...
            return (($settings['site_currency_symbol_position'] == "pre") ? $settings['site_currency_symbol'] : '') . ' ' . number_format($price, $decimal_number_lev) . ' ' . (($settings['site_currency_symbol_position'] == "post") ? $settings['site_currency_symbol'] : '');
    ...
    public function projectNumberFormat($number)
    {
        return '#'.'PROJ' . sprintf("%05d", $number);
   }
    ...
    public static function userWarehouseRegister($user_id)
    {
        warehouse::create(
            [
    ...
    public static function userDefaultBankAccount($user_id)
    {
        BankAccount::create(
            [
                'holder_name' => 'cash',
    ...
    function numberToArabicWords($number, $lang = 'ar')
    {
       ...
       if ($lang == 'ar') {
          ...
          // English words (basic, can be expanded)
          $ones = [
       ...
       if ($number == 0) {
          return ($lang == 'ar') ? 'صفر' : 'zero';
       }

       $parts = [];

       if ($number >= 100000) {
          $hundredThousands = floor($number / 100000);
          $parts[] = $hundreds[$hundredThousands] . (($lang == 'ar') ? ' ألف' : ' thousand');
          $number %= 100000;
       }
       ...
       if ($number >= 1000) {
          ...
          if ($thousands == 1) {
             $parts[] = ($lang == 'ar') ? 'ألف' : 'one thousand';
          } elseif ($thousands == 2) {
             $parts[] = ($lang == 'ar') ? 'ألفان' : 'two thousand';
          } elseif ($thousands <= 10) {
             $parts[] = $ones[$thousands] . (($lang == 'ar') ? ' آلاف' : ' thousand');
          } else {
             $parts[] = $this->numberToArabicWords($thousands, $lang) . (($lang == 'ar') ? ' ألف' : ' thousand');
          }
          $number %= 1000;
       }
       ...
       if ($number > 0) {
          if ($number < 20) {
             ...
             if ($o == 0) {
                ...
                $parts[] = $ones[$o] . (($lang == 'ar') ? ' و' : '-') . $tens[$t];
             }
          }
       }

       return implode(($lang == 'ar') ? ' و ' : ' ', array_filter($parts));
    }

    function convertFormattedAmountToArabic($formatted, $lang = 'ar')
    {
       
    // Define the HTML entity for SAR symbol
    $currency = define('SAR', '<span class="symbol">&#xea;</span>');
    // Remove "SAR", commas, and trim
       $clean = preg_replace('/[^\d.]/', '', $formatted);

       // Split integer and decimal parts
       $parts = explode('.', $clean);
       $riyal = (int)($parts[0] ?? 0);
       $halala = (int)str_pad($parts[1] ?? 0, 2, '0');

       $riyalWords = $this->numberToArabicWords($riyal, $lang) . (($lang == 'ar') ? SAR : ' riyal');

       $halalaWords = '';
       if ($halala > 0) {
          $halalaWords = $this->numberToArabicWords($halala, $lang) . (($lang == 'ar') ? ' هللة' : ' halala');
       }

       return $halalaWords
          ? "$riyalWords " . (($lang == 'ar') ? 'و' : 'and') . " $halalaWords " . (($lang == 'ar') ? 'فقط لا غير' : 'only')
          : "$riyalWords " . (($lang == 'ar') ? 'فقط لا غير' : 'only');
    }
