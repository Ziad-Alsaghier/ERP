    ...
    // dd($plan);
        ...
        if($plan->max_customers > 0){
            $features[] =[
                ...
                'price' => $plan->customers_price,
                'per' => __('For 1000 Customers')
            ];
        }
        if($plan->max_venders > 0){
            $features[] =[
                'id'=> 3,
                'name' => 'Venders',
                'description' => __('Number of Additional Suppliers'),
                'price' => $plan->venders_price,
                'per' => __('For 1000 Venders')
            ];
        }
        if($plan->max_clients > 0){
            $features[] =[
                'id'=> 4,
                'name' => 'Clients',
                'description' => __('Number of Additional Clients'),
                'price' => $plan->clients_price,
                'per' => __('For 1000 Clients')
            ];
        }
@endphp
                            ...
                            <div class="card price-card price-1 wow animate__fadeInUp" data-wow-delay="0.2s" style="
                                ...
                                <div class="card-body">
                                    ...
                                    <h3 class="mb-4 f-w-600  ">
                                        <small class="text-muted">{{(isset($admin_payment_setting['currency_symbol'])) ? $admin_payment_setting['currency_symbol'] : '$'}}</small>
                                        <input type="text" name="total_price_input" id="total_price_input" value="{{ $plan->price }}" hidden>
                                        <input type="text" name="total_price_output" id="total_price_output" value="{{ $plan->price }}" hidden>
                                        <span id="total_price">{{ $plan->price }}</span>
                                        <small class="text-muted">{{  ' / ' . __(\App\Models\Plan::$arrDuration[$plan->duration])}}</small>
                                    </h3>
                                    <ul class="mt-3 text-start" style="list-style: none;">
                                        <li><span><i class="text-primary ti ti-circle-plus me-1"></i> {{ __('Main Profile') }} + {{($plan->max_users==-1)?__('Unlimited'):$plan->max_users}} {{__('Users')}}</span></li>
                                        <li><span><i class="text-primary ti ti-circle-plus me-1"></i>{{($plan->max_customers==-1)?__('Unlimited'):$plan->max_customers}} {{__('Customers')}}</span></li>
                                        <li><span><i class="text-primary ti ti-circle-plus me-1"></i>{{($plan->max_venders==-1)?__('Unlimited'):$plan->max_venders}} {{__('Vendors')}}</span></li>
                                        <li><span><i class="text-primary ti ti-circle-plus me-1"></i>{{($plan->max_clients==-1)?__('Unlimited'):$plan->max_venders}} {{__('Clients')}}</span></li>
                                    </ul>
                                ...
                                @endif
                                    ...
                                    @if (isset($admin_payment_setting['is_paytab_enabled']) && $admin_payment_setting['is_paytab_enabled'] == 'on')
                                        <a href="#paytab_payment"
                                           class="list-group-item list-group-item-action border-0">{{ __('Visa & Master Card') }}
                                            <div class="float-end"><i class="ti ti-chevron-right"></i></div>
                                        </a>
                                    @endif
                    ...
                    @if (isset($admin_payment_setting['is_paytab_enabled']) && $admin_payment_setting['is_paytab_enabled'] == 'on')
                        <div id="paytab_payment" class="card">
                            ...
                            <div class="tab-pane" id="paytab_payment">
                                <form role="form" action="{{ route('plan.pay.with.paytab') }}" method="post" id="paytab-payment-form" class="w3-container w3-display-middle w3-card-4">
                                    @csrf
                                    <input type="hidden" name="total_price" id="paytab_total_price" value="{{ $plan->price }}" class="form-control">
                                    <div class="col-sm-12 my-2 px-2">
                                        <div class="text-end">
                                            <div class="container-fluid">
                                                <div class="row">
                                                ...
                                                <div class="row gy-3 text-start">
                                                    ...
                                                    <div class="col-md-3">
                                                      <label for="cc-month" class="form-label">{{ __('Month of expiration') }}</label>
                                                      <input type="text" class="form-control" name="cc-month" id="cc-month" placeholder="" required>
                                                      <div class="invalid-feedback">
                                                        {{ __('Expiration date required') }}
                                                      </div>
                                                    </div>
                                                    <div class="col-md-3">
                                                      <label for="cc-year" class="form-label">{{ __('Year of expiration') }}</label>
                                                      <input type="text" class="form-control" name="cc-year" id="cc-year" placeholder="" required>
                                                      <div class="invalid-feedback">
                                                        {{ __('Expiration date required') }}
                                                      </div>
                                                    </div>
                                            ...
                                            <input type="hidden" name="plan_id" value="{{ \Illuminate\Support\Facades\Crypt::encrypt($plan->id) }}">
                                            <input type="submit" class="btn btn-primary mb-2 me-3" value="{{ __('Pay Now') }}" id="paytab-get-status">
                                        </div>
                                    </div>
                                </form>
                            </div>
