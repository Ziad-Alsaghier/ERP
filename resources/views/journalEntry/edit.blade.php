...
{{--                                </td>--}}
                                ...
                                <td width="25%" class="form-group pt-0">
                                    {{-- {{ Form::select('account', $accounts,'', array('class' => 'form-control js-searchBox','required'=>'required')) }} --}}
                                    <select name="account" class="form-control select2" id="accounts" required="required">
                                        @foreach ($chartAccounts as $chartAccount)
                                            <option value="{{ $chartAccount['id'] }}" class="subAccount">{{ __($chartAccount['code_name']) }}</option>
                                            @foreach ($subAccounts as $subAccount)
                                                @if ($chartAccount['id'] == $subAccount['account'])
                                                    <option value="{{ $subAccount['id'] }}" class="ms-5"> &nbsp; &nbsp;&nbsp; {{ __($subAccount['code_name']) }}</option>
                                                @endif
                                            @endforeach
                                        @endforeach
                                    </select>
