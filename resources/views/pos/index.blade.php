    ...
    $( document ).ready(function() {
        ...
        $(document).on('keyup', '.discount', function () {
            ...
            $.ajax({
                ...
                error: function (data) {
                    data = data.responseJSON;
                    show_toastr('{{ __("Error") }}', Array.isArray(data.error) ? data.error.join('<br>') : data.error, 'error');
                    {{--                    muhamed show_toastr('{{ __("Error") }}', data.error, 'error'); --}}
                }
            });
...
<script>
{{--  muhamed   var site_currency_symbol_position = '{{ \App\Models\Utility::getValByName('site_currency_symbol_position') }}'; --}}
var site_currency_symbol_position = '{{ is_array($position = \App\Models\Utility::getValByName("site_currency_symbol_position")) ? implode(",", $position) : $position }}';
