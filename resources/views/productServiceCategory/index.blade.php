                        ...
                        <table class="table datatable">
                            ...
                            @foreach ($categories as $category)
                                <tr>
                                    ...
                                    <td class="font-style">{{ $category->parent?->name ??  "Parent" }}</td>
                                    <td class="font-style">
                                        {{ __(\App\Models\ProductServiceCategory::$catTypes[$category->type]) }}
                                    </td>
                                    <td>{{ (!empty($category->chartAccount)?$category->chartAccount->name :'-') }}</td>
                                    <td>        <img id="preview" src="{{ $category->image ? asset('storage/app/public/' . $category->image) : '' }}" alt="Preview" style="max-width: 50px; max-height: 200px;"></td>
                                    ...
                                    <td class="Action">
                                        ...
                                        @can('edit constant category')
                                                <div class="action-btn bg-primary ms-2">
                                                    <a href="#" class="mx-3 btn btn-sm align-items-center" data-url="{{ route('product-category.edit',$category->id) }}" data-ajax-popup="true" data-title="{{__('Edit Product Category')}}" data-bs-toggle="tooltip" title="{{__('Edit')}}" data-original-title="{{__('Edit')}}">
