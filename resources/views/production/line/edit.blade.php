{{ Form::model($line, [
    'route' => ['production.line.update', $line->id],
    'method' => 'PUT',
...
<div class="modal-body">
    ...
    <div id="step1-section" class="step-section">
        <div class="row">
            ...
            <div class="col-md-6">
                <div class="form-group">
                    ...
                    {{ Form::text('name',$line->name, ['class' => 'form-control', 'placeholder' => 'Enter Product Name', 'required']) }}
                    <div class="invalid-feedback">{{ __('Please Enter Product Name') }}.</div>
                </div>
            </div>
            {{--  Branchs Select --}}
            <div class="form-group col-md-6">
                {{ Form::label('branchs', __('Branchs'), ['class' => 'form-label']) }}
                {{ Form::select('branch_id', $branchs, $line->branch->id, ['class' => 'form-control', 'required'])   }}
            </div>
            {{--  Branchs Select --}}
        ...
        {{ Form::select('type_id', $types, $line->type_id, ['class' => 'form-control', 'required']) }}
            </div>
            {{--  Type Section --}}
            ...
            <div class="form-group col-md-12">
                <div class="form-group">
                    ...
                    <label class="custom-switch">
                        <input type="checkbox" name="is_enabled" value='{{ $line->is_enabled == '0' ?  '1' : null}}'  {{ $line->is_enabled == '1' ? 'checked' : ''}}>
    ...
    <div id="step2-section" class="step-section" style="display: none;">
        <div class="row">
            ...
            {{ Form::select('product_id', $products, optional($line->linProducts->first())->product_id, ['class' => 'form-control', 'required']) }}
            </div>
            {{--<div class="form-group col-md-6">
                {{ Form::label('accounts', __('Accounts'), ['class' => 'form-label']) }}<span
                    class="text-danger">*</span>
                {{ Form::select('cost_center', $line, null, ['class' => 'form-control', 'required']) }}
            </div>--}}
            <div class="form-group col-md-6">
                {{ Form::label('operators', __('Operators'), ['class' => 'form-label']) }}
                {{ Form::select('employee_id', $operators, optional($line->operators->first())->employee_id, ['class' => 'form-control', 'required']) }}
            </div>
