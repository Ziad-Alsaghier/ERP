                ...
                <div class="card-body">
                    <div class="d-flex align-items-center">
                        <div class="avatar me-3">
                            <img {{ $project->img_image }} alt="" class="img-user">
                        </div>
                            ...
                            @php
                                ...
                                if (is_array($descriptionArray)) {
                                    ...
                                    foreach ($descriptionArray as $key => $data) {
                                        ...
                                        $namearr = json_decode($data['description'],true);
                                        if($namearr != null){
                                            // print_r($namearr['info']);
                                            $name = $namearr['info']['name'];
                                        }else{
                                            $productname = App\Models\ProductService::find($data['product_id']);
                                        ...
                                        $descriptionJson .= '<div class="row alert alert-info" style="direction: rtl;">';
                                            $descriptionJson .= '<div class="col-6 alert alert-danger" style="text-align:center;direction: rtl;" role="alert">' . ' اسم المنتج'.'</div>';
                                            $descriptionJson .= '<div class="col-6 alert alert-success" style="text-align:center;" role="alert">' . $name.'</div>';
                                            $descriptionJson .= '<div class="col-6 alert alert-danger" style="text-align:center;direction: rtl;" role="alert">' . ' الكمية'.'</div>';
                                            $descriptionJson .= '<div class="col-6 alert alert-success" style="text-align:center;direction: rtl;" role="alert">' . $data['quantity'].'</div>';
                                            if(!empty($data['description']) && $namearr != null){
                                                // dd($namearr['attachment']);
                                                $descriptionJson .= '<div class="col-6 alert alert-danger" style="text-align:center;direction: rtl;" role="alert">' . __('description') .'</div>';
                                                $descriptionJson .= '<div class="col-6 alert alert-success" style="text-align:center;direction: rtl;" role="alert">';
                                                    if(isset($namearr['attachment'])&& $namearr['attachment'] != null){
                                                        $descriptionJson .= '<a target="__blank" class="btn btn-primary btn-sm mb-3" href="'.url("storage/".$namearr["attachment"]).'">'.__("view attachment").' <i class="ti ti-link"></i></a><br>';
                                                    }
                                                    $descriptionJson .= '<a target="__blank" href="' . route('projects.pdf', $task_id . '?task='.$data['id']) .'" class="btn btn-primary btn-sm">'. __('work order') .'<i class="ti ti-eye"></i></a>';
                                                    $descriptionJson .= '</div>';
                                                }
                                                if(isset($namearr['design_note'])&& $namearr['design_note'] != null){
                                                    $descriptionJson .= '<div class="col-6 alert alert-danger" style="text-align:center;direction: rtl;" role="alert">' . __('design note').'</div>';
                                                    $descriptionJson .= '<div class="col-6 alert alert-success" style="text-align:center;direction: rtl;" role="alert"><textarea style="width: 100%;" readonly>' . $namearr['design_note'].'</textarea></div>';
                                                }
                                                $descriptionJson .= '</div>';
                                                $descriptionJson .= '<hr>';
