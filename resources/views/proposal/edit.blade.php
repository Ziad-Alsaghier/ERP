...
@push('script-page')
    ...
    <script>
        var selector = "body";
        var itemCounter = 1; // متغير لتتبع العدد الحالي للعناصر المكررة
        ...
        if ($(selector + " .repeater").length) {
            ...
            var $repeater = $(selector + ' .repeater').repeater({
                    ...
                    show: function () {
                        $(this).slideDown();
                            // إضافة الأرقام التسلسلية إلى الـ ID
                        $(this).find('.item').attr('id', 'item' + itemCounter);
                        itemCounter++; // زيادة العداد بعد تعيين ID
                    ...
                    $.ajax({
                        ...
                        success: function (data) {
                            var proposalItems = JSON.parse(data);
                                ...
                                $('.item').addClass('select2');

                            if (proposalItems != null) {
                                var amount = (proposalItems.price * proposalItems.quantity);
                                $(el.parent().parent().parent().parent().find('.quantity')).val(proposalItems.quantity);
                                $(el.parent().parent().parent().parent().find('.price')).val(proposalItems.price);
                                $(el.parent().parent().parent().parent().find('.discount')).val(proposalItems.discount);
                                $(el.parent().parent().parent().parent().find('.pro_description')).val(proposalItems.description);
                                // $('.pro_description').text(p.parent()roposalItems.description);
                            ...
                            var discount=$(el.parent().parent().parent().parent().find('.discount')).val();
                            ...
                            for (var j = 0; j < itemTaxPriceInput.length; j++) {
                                ...
                                if (proposalItems != null) {
                                    $(el.parent().parent().parent().parent().find('.amount')).html(parseFloat(amount)+parseFloat(itemTaxPrice)-parseFloat(discount));
                                } else {
                                    $(el.parent().parent().parent().parent().find('.amount')).html(parseFloat(item.totalAmount)+parseFloat(itemTaxPrice));
                                }
        ...
        var customerId = '{{$proposal->customer_id}}';
                            ...
                            <div class="row">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        ...
                                        <div class="form-icon-user">
                                            {{ Form::input('datetime-local', 'due_date',null, ['class' => 'form-control',]) }}
                                        </div>
                                    </div>
                                </div>
                                <div class="col-md-6">
                                ...
                                <div class="col-md-6">
                                    <div class="form-group">
                                        {{ Form::label('proposal_details', __('Proposal Details'),['class'=>'form-label']) }}
                                        <div class="form-icon-user">
                                            <textarea class="form-control" name="proposal_details">{{$proposal->proposal_details}}</textarea>
                                        </div>
                                    </div>
                                </div>
                                ...
                                <div class="col-md-6">
                                    <div class="form-check form-switch">
                                        <input type="checkbox" class="form-check-input" id="make_project" name="make_project"  {{ $proposal->project == 1 ? 'checked' : '' }} />
                                        <label class="form-check-label f-w-600 pl-1" for="make_project" >{{ __('Create Project') }}</label>
                                    </div>
                                </div>
                        ...
                        <table class="table mb-0" data-repeater-list="items" id="sortable-table">
                            ...
                            <tr>
                                ...
                                <td width="25%" class="form-group pt-0">
                                    {{ Form::select('item', $product_services,null, array('class' => 'form-control item','id'=>'item','data-url'=>route('proposal.product'))) }}
                                </td>
                                <td>
