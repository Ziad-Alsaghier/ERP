...
<script>
    ...
    $('.summernote-simple5').on('summernote.blur', function() {
        ...
        $.ajax({
            ...
            data: {
                _token: $('meta[name="csrf-token"]').attr('content'),
                header_bill: $(this).val()
            },
            type: 'POST',
            success: function(response) {
                show_toastr('success', __('Setting successfully updated.'), 'success');
    ...
    $('.summernote-simple-invoice-footer').on('summernote.blur', function() {

        $.ajax({
            url: "{{ route('company.settings') }}",
            data: {
                _token: $('meta[name="csrf-token"]').attr('content'),
                invoice_footer: $(this).val()
            },
            type: 'POST',
            success: function(response) {
                show_toastr('success', __('Setting successfully updated.'), 'success');
    ...
    $('.summernote-simple-proposal-footer').on('summernote.blur', function() {

        $.ajax({
            url: "{{ route('company.settings') }}",
            data: {
                _token: $('meta[name="csrf-token"]').attr('content'),
                proposal_footer: $(this).val()
            },
            type: 'POST',
            success: function(response) {
                show_toastr('success', __('Setting successfully updated.'), 'success');
...
$(function() {
    $('#generateApi').on('click', function(e) {
        e.preventDefault();
        const apiKey_input = $('input[name="api_key"]');
        Swal.fire({
            ...
            preConfirm: async (password) => {
                try {
                    const response = await fetch("{{ route('generate.apis') }}", {
                        method: "POST",
                        headers: {
                    ...
                    if (!response.ok) {
                        ...
                        return Swal.showValidationMessage(error.message || "An error occurred");
                    }


                    const data = await response.json();
                    return data; // هيرجع الكي من السيرفر
                } catch (error) {
            ...
            if (result.isConfirmed && result.value) {
                Swal.fire({
                    title: `{{ __("API Key generated successfully.")}}`
                });
                apiKey_input.val(result.value.api_key); // هنا نحط القيمة اللي رجعت من السيرفر
            }
        });
    });

    $('#copyApi').on('click', function(e) {
        e.preventDefault();
            ...
            <div class="col-xl-3">
                <div class="card sticky-top" style="top:30px">
                    <div class="list-group list-group-flush" id="useradd-sidenav">
                        ...
                        <a href="#api-settings"
                            class="list-group-item list-group-item-action border-0">{{ __('APIs Settings') }}
                            <div class="float-end"><i class="ti ti-chevron-right"></i></div>
                        </a>
            ...
            <div class="col-xl-9">
                <div id="accounting-settings" class="card">
                    <div class="card-header">
                        ...
                        <small class="text-muted">{{ __('Edit your Accounting settings') }}</small>
                    </div>
                    {{ Form::model($setting, ['route' => 'accounting.settings', 'method' => 'post']) }}
                    <div class="card-body">
                        <div class="row">
                            <div class="form-group col-md-4">
                                <label class="form-control-label">{{ __('Close Year Date') }}</label> <br>
                                <small>{{ __('Select the date to close the accounting year.') }}</small>
                                {{ Form::date('close_year_date', isset($setting['close_year_date']) ? $setting['close_year_date'] : null, ['class' => 'form-control', 'placeholder' => __('Select Close Year Date')]) }}
                            </div>
                            <div class="form-group col-md-4">
                                <label class="form-control-label">{{ __('Start of Accounting Year') }}</label> <br>
                                <small>{{ __('Select the day and month for the start of the accounting year.') }}</small>

                                <div class="d-flex gap-2">
                                    {{-- Day Dropdown --}}
                                    {{ Form::select('start_year_day', range(1, 31), isset($setting['start_year_day']) ? $setting['start_year_day'] : null, ['class' => 'form-control', 'placeholder' => __('Day')]) }}

                                    {{-- Month Dropdown --}}
                                    {{ Form::select('start_year_month', [
                        '01' => 'January', '02' => 'February', '03' => 'March',
                        '04' => 'April', '05' => 'May', '06' => 'June',
                        '07' => 'July', '08' => 'August', '09' => 'September',
                        '10' => 'October', '11' => 'November', '12' => 'December'
                    ], isset($setting['start_year_month']) ? $setting['start_year_month'] : null, ['class' => 'form-control', 'placeholder' => __('Month')]) }}
                                </div>
                            </div>
                            <div class="form-group col-md-4">
                                <label class="form-control-label">{{ __('Select Inventory Cost Way') }}</label> <br>
                                <small>{{ __('Choose a method for inventory cost calculation') }}</small>
                                {{ Form::select('inventory_cost_way', ['fifo' => 'FIFO', 'lifo' => 'LIFO', 'average' => 'Average'], isset($setting['inventory_cost_way']) ? $setting['inventory_cost_way'] : 'fifo', ['class' => 'form-control']) }}
                            </div>
                        </div>
                    </div>
                    ...
                    <div class="card-body">
                        <div class="row">
                            ...
                            <div class="row">
                                <div class="form-group col-md-4">
                                    @php
                                            $api_key = App\Models\ApiKeys::where('tokenable_id', Auth::user()->id)->orderBy('created_at', 'desc')->first();
                                    @endphp
                                    {{ Form::text('api_key', $api_key->token ?? '', ['class' => 'form-control ', 'readonly']) }}
                                </div>
                                <div class="col-4">
                                    <button class="btn btn-primary " id="generateApi">{{ __('Generate') }}</button>
                                    <button class="btn btn-secondary " id="copyApi">{{ __('Copy') }}</button>
                                </div>
                            </div>
                        </div>
                    </div>
                ...
                {{-- bill Settings --}}
                <div id="bill-settings" class="card">
                    <div class="card-header">
                        <h5>{{ __('Invoice Print Setting') }}</h5>
                        <small class="text-muted">{{ __('Edit your Invoice Setting') }}</small>
                    </div>
                    {{ Form::model($setting, ['route' => 'company.settings', 'method' => 'post']) }}
                    <div class="card-body">
                        <div class="row">
                            <div class="form-group col-md-6">
                                {{ Form::label('company_name_bill *', __('Company Name *'), ['class' => 'form-label']) }}
                                {{ Form::text('company_name_bill', null, ['class' => 'form-control font-style']) }}
                                @error('company_name_bill')
                                <span class="invalid-company_name_bill" role="alert">
                                    <strong class="text-danger">{{ $message }}</strong>
                                </span>
                                @enderror
                            </div>
                            <div class="form-group col-md-6">
                                {{ Form::label('company_address_bill', __('Address'), ['class' => 'form-label']) }}
                                {{ Form::text('company_address_bill', null, ['class' => 'form-control font-style']) }}
                                @error('company_address_bill')
                                <span class="invalid-company_address_bill" role="alert">
                                    <strong class="text-danger">{{ $message }}</strong>
                                </span>
                                @enderror
                            </div>
                            <div class="form-group col-md-6">
                                {{ Form::label('company_city_bill', __('City'), ['class' => 'form-label']) }}
                                {{ Form::text('company_city_bill', null, ['class' => 'form-control font-style']) }}
                                @error('company_city_bill')
                                <span class="invalid-company_city_bill" role="alert">
                                    <strong class="text-danger">{{ $message }}</strong>
                                </span>
                                @enderror
                            </div>
                            <div class="form-group col-md-6">
                                {{ Form::label('company_state_bill', __('State'), ['class' => 'form-label']) }}
                                {{ Form::text('company_state_bill', null, ['class' => 'form-control font-style']) }}
                                @error('company_state_bill')
                                <span class="invalid-company_state_bill" role="alert">
                                    <strong class="text-danger">{{ $message }}</strong>
                                </span>
                                @enderror
                            </div>
                            <div class="form-group col-md-6">
                                {{ Form::label('company_zipcode_bill', __('Zip/Post Code'), ['class' => 'form-label']) }}
                                {{ Form::text('company_zipcode_bill', null, ['class' => 'form-control']) }}
                                @error('company_zipcode_bill')
                                <span class="invalid-company_zipcode_bill" role="alert">
                                    <strong class="text-danger">{{ $message }}</strong>
                                </span>
                                @enderror
                            </div>
                            <div class="form-group  col-md-6">
                                {{ Form::label('company_country_bill', __('Country'), ['class' => 'form-label']) }}
                                {{ Form::text('company_country_bill', null, ['class' => 'form-control font-style']) }}
                                @error('company_country_bill')
                                <span class="invalid-company_country_bill" role="alert">
                                    <strong class="text-danger">{{ $message }}</strong>
                                </span>
                                @enderror
                            </div>
                            <div class="form-group col-md-6">
                                {{ Form::label('company_telephone_bill', __('Telephone'), ['class' => 'form-label']) }}
                                {{ Form::text('company_telephone_bill', null, ['class' => 'form-control']) }}
                                @error('company_telephone_bill')
                                <span class="invalid-company_telephone_bill" role="alert">
                                    <strong class="text-danger">{{ $message }}</strong>
                                </span>
                                @enderror
                            </div>
                            <div class="form-group col-md-6">
                                {{ Form::label('company_telephone_bill_another', __('sales Telephone'), ['class' => 'form-label']) }}
                                {{ Form::text('company_telephone_bill_another', null, ['class' => 'form-control']) }}
                                @error('company_telephone_bill_another')
                                <span class="invalid-company_telephone_bill_another" role="alert">
                                    <strong class="text-danger">{{ $message }}</strong>
                                </span>
                                @enderror
                            </div>
                            <div class="form-group col-md-6">
                                {{ Form::label('email_bill', __('Email'), ['class' => 'form-label']) }}
                                {{ Form::text('email_bill', null, ['class' => 'form-control']) }}
                                @error('email_bill')
                                <span class="invalid-email_bill" role="alert">
                                    <strong class="text-danger">{{ $message }}</strong>
                                </span>
                                @enderror
                            </div>
                            <div class="form-group col-md-6">
                                {{ Form::label('email_bill_another', __('sales Email'), ['class' => 'form-label']) }}
                                {{ Form::text('email_bill_another', null, ['class' => 'form-control']) }}
                                @error('email_bill_another')
                                <span class="invalid-email_bill_another" role="alert">
                                    <strong class="text-danger">{{ $message }}</strong>
                                </span>
                                @enderror
                            </div>
                            <hr>
                            <h4>{{__('Header information')}}</h4>
                            <div class="form-group col-md-6">
                                {{ Form::label('header_bill', __('social links OR Data'), ['class' => 'form-label']) }}
                                <textarea class="summernote-simple5 summernote-simple-3">@if(isset($setting['header_bill'])){!! $setting['header_bill'] !!}@endif</textarea>
                            </div>
                            <div class="col-md-6">
                                {{ Form::label('social_link_bill', __('QR Code'), ['class' => 'form-label']) }}
                                <textarea name="social_link_bill" rows="5" style="width: 100%;padding: 14px;">@if(isset($setting['social_link_bill'])){!! $setting['social_link_bill'] !!}@endif</textarea>
                                @error('social_link_bill')
                                <span class="invalid-social_link_bill" role="alert">
                                    <strong class="text-danger">{{ $message }}</strong>
                                </span>
                                @enderror
                                @if (!empty($setting['social_link_bill']))
                                {!! DNS2D::getBarcodeHTML($setting['social_link_bill'], "QRCODE" , 2,2) !!}
                                @endif
                            ...
                            <div class="form-group col-md-6">
                                {{ Form::label('invoice_footer', __('invoice footer'), ['class' => 'form-label']) }}
                                <textarea class="summernote-simple-invoice-footer summernote-simple-3">@if(isset($setting['invoice_footer'])){!! $setting['invoice_footer'] !!}@endif</textarea>
                            </div>
                            <div class="form-group col-md-6">
                                {{ Form::label('proposal_footer', __('proposal footer'), ['class' => 'form-label']) }}
                                <textarea class="summernote-simple-proposal-footer summernote-simple-3">@if(isset($setting['proposal_footer'])){!! $setting['proposal_footer'] !!}@endif</textarea>
                            </div>
                ...
                {{-- Zatca Settings --}}
                <div id="Zatca-settings" class="card">
                    <div class="card-header">
                        <h5>{{ __('Zatca Settings') }}</h5>
                        <div class="row">
                            <small class="text-muted">{{ __('Edit your Zatca Setting') }}</small>
                        </div>
                    </div>
                    {{ Form::model($setting, ['route' => 'company.settings', 'method' => 'post']) }}
                    <div class="card-body">
                        <div class="row">
                            <div class="form-group col-md-6">
                                {{ Form::label('sellerName *', __('sellerName *'), ['class' => 'form-label']) }}
                                {{ Form::text('sellerName', null, ['class' => 'form-control font-style']) }}
                                @error('sellerName')
                                <span class="invalid-sellerName" role="alert">
                                    <strong class="text-danger">{{ $message }}</strong>
                                </span>
                                @enderror
                            </div>
                            <div class="form-group col-md-6">
                                {{ Form::label('vatRegistrationNumber *', __('vatRegistrationNumber *'), ['class' => 'form-label']) }}
                                {{ Form::number('vatRegistrationNumber', null, ['class' => 'form-control font-style','max'=>'399999999999993','min'=>'300000000000003'])}}
                                {{-- <input type="number" class="form-control font-style" name="vatRegistrationNumber" min="15" max="15"> --}}
                                @error('vatRegistrationNumber')
                                <span class="invalid-vatRegistrationNumber" role="alert">
                                    <strong class="text-danger">{{ $message }}</strong>
                                </span>
                                @enderror
                            </div>
                            {{-- <div class="form-group col-md-6">
                                    {{ Form::label('API *', __('API *'), ['class' => 'form-label']) }}
                            {{ Form::text('API', null, ['class' => 'form-control font-style']) }}
                            @error('API')
                            <span class="invalid-API" role="alert">
                                <strong class="text-danger">{{ $message }}</strong>
                            </span>
                            @enderror
                        </div> --}}
