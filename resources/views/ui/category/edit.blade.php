...
{{ Form::model($category, ['route' => ['design.change.update', $category->id], 'method' => 'PUT', 'enctype' => 'multipart/form-data']) }}
<div class="modal-body">
    <div class="row px-2">
        ...
        <div class="form-group col-md-12 animated-group">
            ...
            {{ Form::select('cat_id', $categories, $category->id, ['class' => 'form-control custom-select', 'id' => 'category_id', 'required']) }}
        </div>

        {{-- Enabled Switch --}}
        <div class="form-group col-md-6">
            <label class="animated-label d-block mb-2">{{ __('Status') }}</label>
            <label class="switch">
                <input type="checkbox" name="is_enabled" value="1" {{ $category->is_enabled ? 'checked' : '' }}>
                <span class="slider round"></span>
                <span class="switch-label">
                    {{ $category->is_enabled ? __('Enabled') : __('Disabled') }}
                </span>
            </label>
        </div>
        ...
        <div class="form-group col-md-6 animated-group">
            ...
            {{ Form::select('section', ['header' => 'Header', 'footer' => 'Footer'], $category->section, ['class' => 'form-control custom-select', 'id' => 'position', 'required']) }}
        </div>
...
document.querySelectorAll('.switch input[type="checkbox"]').forEach(function(checkbox) {
    checkbox.addEventListener('change', function () {
        ...
        label.textContent = this.checked ? '1' : '0';
    });
});
</script>
