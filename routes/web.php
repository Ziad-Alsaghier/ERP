...
use App\Http\Controllers\MeetTrackController;
use App\Http\Controllers\shippingController;
use App\Http\Controllers\TestingController;
use App\Http\Controllers\webOrdersController;
...
// Invoice Payment Gateways
Route::post('customer/{id}/payment', [CheckoutPaymentController::class, 'addpayment'])->name('customer.payment');
Route::get('invoice/pdf/{id}', [InvoiceController::class, 'invoice'])->name('invoice.pdf')->middleware(['XSS', 'revalidate']);
Route::get('revenue/pdf/{id}', [RevenueController::class, 'revenue'])->name('revenue.pdf')->middleware(['XSS']);
...
Route::group(['middleware' => ['verified','company']], function () {
    ...
    Route::any('user-confirm/{id}', [UserController::class, 'confirmaccount'])->name('users.confirm');
    ...
    Route::get('/productservice/attributes/ajax', [ProductServiceController::class, 'ajaxList'])->name('productservice.attributes.ajax')->middleware(['auth', 'XSS']);
    ...
    Route::get('productservice/json/list', [ProductServiceController::class, 'datajson'])->name('productservice.datajson')->middleware(['auth', 'XSS', 'revalidate']);
    //Product Stock
    ...
    Route::group(
        [
            ...
            Route::post('invoice/product/destroy', [InvoiceController::class, 'productDestroyJson'])->name('invoice.product.destroy');
    ...
    Route::group(
        [
            ...
            Route::post('custom-credit-note', [CreditNoteController::class, 'custom_Store'])->name('invoice.custom.credit.note');
    ...
    Route::get('payment/pdf/{id}', [PaymentController::class, 'payment'])->name('payment.pdf')->middleware(['XSS', 'revalidate']);
    ...
    Route::group(
        [
            ...
            Route::get('report/balance-sheet-report/{view?}/{collapseview?}', [ReportController::class, 'balanceSheet'])->name('report.balance.sheet');
            Route::get('report/profit-loss-report/{view?}/{collapseView?}', [ReportController::class, 'profitLoss'])->name('report.profit.loss');

            Route::get('ledger-report/{account?}', [ReportController::class, 'ledgerSummary'])->name('report.ledger');
            Route::get('report/trial-balance-report/{view?}', [ReportController::class, 'trialBalanceSummary'])->name('trial.balance');
            ...
            Route::get('reports-date-cashflow', [ReportController::class, 'dateCashflow'])->name('report.date.cashflow')->middleware(['auth', 'XSS']);
            ...
            Route::get('reports/', [ReportController::class, 'reports'])->name('report.financial_reports');
            Route::post('/reports/toggle', [ReportController::class, 'toggleReportStatus'])->name('report.toggle');
    ...
    Route::group(
        [
            ...
            Route::get('proposal/{id}/project', [ProposalController::class, 'project'])->name('proposal.project');
            ...
            Route::get('proposal/json/list', [ProposalController::class, 'datajson'])->name('proposal.datajson');

            Route::post('proposal/workorder', [ProposalController::class, 'workorder'])->name('proposal.workorder');
    ...
    //addedd by muhammed for minitype ajax
    // Route::post('chart-of-account/minitype', [ChartOfAccountController::class, 'getMiniType'])->name('charofAccount.miniType')->middleware(['auth', 'XSS', 'revalidate']);
    //end added mini type
    ...
    Route::get('/{id}/notification/seen', [UserController::class, 'notificationSeen'])->name('notification.seen');
    ...
    Route::get('projects/pdf/{id}', [ProjectController::class, 'pdf'])->name('projects.pdf')->middleware(['XSS', 'revalidate']);
...
//    Route::post('/LandingPage/copySection', [LandingPageSectionController::class, 'copySection'])->middleware(['auth', 'XSS']);
    ...
    // Route::post('plan-pay-with-paytab', [PaytabController::class, 'planPayWithpaytab'])->middleware(['auth'])->withoutMiddleware('company')->name('plan.pay.with.paytab');
    // Route::post('plan-pay-with-paytab', [PaytabController::class, 'planPayWithpaytab'])
    // ->middleware(['auth']) // Applies the 'auth' middleware
    // ->withoutMiddleware('company') // Removes 'company' middleware
    // ->name('plan.pay.with.paytab');
...
//    Route::get('{id}/plan-get-payment-status', [PaypalController::class, 'planGetPaymentStatus'])->name('plan.get.payment.status')->middleware(['auth', 'XSS', 'revalidate']);
    ...
    Route::get('meet-track', [MeetTrackController::class, 'index'])->name('meet.index')->middleware(['auth', 'XSS']);
    ...
    //================================= Manufacturing ====================================//
    Route::get('/manufacturing', [ManufacturingController::class, 'index'])->name('Manufacturing')->middleware(['auth', 'XSS']);
    Route::get('/instant-quote', [ManufacturingController::class, 'instantquote'])->name('instant-quote')->middleware(['auth', 'XSS']);
    Route::get('/manufacturing/create', [ManufacturingController::class, 'create'])->name('manufacturing.create')->middleware(['auth', 'XSS']);
    ...
    Route::prefix('production')->group(function(){
       ...
       // Type Routes
        Route::prefix('type')->group(function(){
            ...
            Route::get('/edit/{productionLineType}', [ProductionLineController::class, 'productionLineTypeEdit'])->name('production.line.type.edit')->middleware(['auth', 'XSS']);
            Route::PUT('/update/{productionLineType}', [ProductionLineController::class, 'productionLineTypeUpdate'])->name('production.line.type.update')->middleware(['auth', 'XSS']);
            Route::delete('/delete/{id}', [ProductionLineController::class, 'productionTypeDelete'])->name('production.line.type.delete')->middleware(['auth', 'XSS']);
        });
    });
    //================================= Production Lines ====================================//
    ...
    Route::post('generate.apis', [ApiGenerate::class, 'generate_api'])->name('generate.apis');
});
...
Route::group(['middleware' => ['ghost','auth','XSS', 'revalidate']], function () {
    ...
    Route::any('edit-profile', [UserController::class, 'editprofile'])->name('update.account');
    Route::get('order', [CheckoutPaymentController::class, 'index'])->name('order.index');
    Route::get('/checkout/{code}', [CheckoutPaymentController::class, 'stripe'])->name('checkout');
    Route::post('/checkout', [CheckoutPaymentController::class, 'stripePost'])->name('checkout.post');
